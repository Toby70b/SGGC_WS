...
public class SteamIdValidator extends StringValidator {
    ...
    @Override
    public ValidationResult validate(String steamId) {
        if (steamId.length() != STEAM_USER_ID_LENGTH) {
            return new ValidationResult(true, steamId, USER_ID_INVALID_LENGTH);
        }
        if (!StringUtils.isNumeric(steamId)) {
            return new ValidationResult(true, steamId, USER_ID_IS_NOT_NUMERIC_ERROR_MESSAGE);
        }
